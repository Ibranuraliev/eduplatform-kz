...
const api = axios.create({
  baseURL: process.env.REACT_APP_API_URL || 'https://eduplatform-kz-production.up.railway.app/api',
...
api.interceptors.response.use(
  ...
  async err => {
    ...
    if (err.response?.status === 401 && !original._retry) {
      ...
      try {
        const res = await axios.post(`${process.env.REACT_APP_API_URL || 'https://eduplatform-kz-production.up.railway.app/api'}/users/token/refresh/`, {
